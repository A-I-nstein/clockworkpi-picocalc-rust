...
use panic_halt as _;
use rp235x_hal::{
    I2C, Sio, Timer, Watchdog, block::ImageDef, clocks, entry, fugit::RateExtU32, gpio::Pins,
    pac::Peripherals, usb::UsbBus,
};
...
const KB_ADDR: u8 = 0x1f;
const KB_ID_REG: u8 = 0x09;
...
fn main() -> ! {
    ...
    loop {
        ...
        if current_time_us - last_send_time_us >= 1_000_000 {
            ...
            match i2c.write_read(KB_ADDR, &[KB_ID_REG], &mut kb_value) {
                Ok(()) => {
                    let _ = serial.write(&kb_value[0..]);
                    let _ = serial.flush();
                }
                Err(e) => {
                    write!(writer, "{:?}\r\n", e).unwrap();
